  ...
  class PaymentPaidCheque
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PaymentPaidCheque` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PaymentPaidCheque`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [payment_structure_type, document_id, payment_method, payment_date, collected, withheld_percentage, withheld_amount, cheque_date, cheque_number, cheque_bank_account_id, bank_account_id, payment_charge, payment_remarks, remaining_collected_type, remaining_collected].hash
    end
