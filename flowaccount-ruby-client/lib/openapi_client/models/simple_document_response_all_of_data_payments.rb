  ...
  class SimpleDocumentResponseAllOfDataPayments
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SimpleDocumentResponseAllOfDataPayments` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SimpleDocumentResponseAllOfDataPayments`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [payment_date, collected, payment_deduction_type, payment_deduction_amount, withheld_percentage, withheld_amount, payment_charge, payment_method, payment_channel, bank_account_type, bank_account_name, bank_account_number, cheque_date, cheque_number, credit_card_bank_account_id, payment_remarks, remaining_collected_type, remaining_collected].hash
    end
