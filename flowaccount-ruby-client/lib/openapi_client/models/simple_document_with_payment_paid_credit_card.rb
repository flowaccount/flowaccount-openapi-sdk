  ...
  class SimpleDocumentWithPaymentPaidCreditCard
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SimpleDocumentWithPaymentPaidCreditCard` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SimpleDocumentWithPaymentPaidCreditCard`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [contact_code, contact_name, contact_address, contact_tax_id, contact_branch, contact_person, contact_email, contact_number, contact_zip_code, contact_group, published_on, credit_type, credit_days, due_date, sales_name, project_name, reference, is_vat_inclusive, use_receipt_deduction, sub_total, discount_percentage, discount_amount, total_after_discount, is_vat, vat_amount, grand_total, document_show_withholding_tax, document_withholding_tax_percentage, document_withholding_tax_amount, document_deduction_type, document_deduction_amount, remarks, internal_notes, show_signature_or_stamp, items, document_reference, document_payment_structure_type, payment_method, payment_date, collected, withheld_percentage, withheld_amount, credit_card_bank_account_id, payment_remarks, remaining_collected_type, remaining_collected].hash
    end
