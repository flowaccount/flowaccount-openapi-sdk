  ...
  class InlineDocumentResponseAllOfData
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::InlineDocumentResponseAllOfData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::InlineDocumentResponseAllOfData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [record_id, document_id, document_serial, contact_code, contact_name, contact_address, contact_tax_id, contact_branch, contact_person, contact_email, contact_number, contact_zip_code, contact_group, published_on, credit_type, credit_days, due_date, sales_name, project_name, reference, is_vat_inclusive, discount_type, use_inline_discount, use_inline_vat, use_receipt_deduction, sub_total, discount_percentage, discount_amount, total_after_discount, total_without_vat, exempt_amount, vatable_amount, is_vat, vat_amount, grand_total, document_show_withholding_tax, document_withholding_tax_percentage, document_deduction_type, document_deduction_amount, remarks, internal_notes, show_signature_or_stamp, items, referenced_to_me, referenced_by_me, payments, company, status, status_string, document_type, allow_delete].hash
    end
