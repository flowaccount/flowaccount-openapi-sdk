...
> \OpenAPI\Client\Model\InlineDocumentResponse purchasesOrdersIdPut($authorization, $id, $inline_document)
...
try {
    $result = $apiInstance->purchasesOrdersIdPut($authorization, $id, $inline_document);
    print_r($result);
} catch (Exception $e) {
